...
const moralisStreams = (parse, express, options) =>
...
const prepareSyncs = async (parseObject, syncs) => {
  ...
  for (const sync of syncs) {
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("Txs"),
      { fields: { transaction_index: {}, transaction_hash: {} } },
      ["transaction_index", "transaction_hash"]
    );
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("Logs"),
      { fields: { log_index: {}, transaction_hash: {} } },
      ["log_index", "transaction_hash"]
    );
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("ERC20Transfers"),
      { fields: { log_index: {}, transaction_hash: {} } },
      ["log_index", "transaction_hash"]
    );
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("ERC20Approvals"),
      { fields: { log_index: {}, transaction_hash: {} } },
      ["log_index", "transaction_hash"]
    );
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("NFTTransfers"),
      { fields: { log_index: {}, transaction_hash: {} } },
      ["log_index", "transaction_hash"]
    );
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("ERC721Approvals"),
      { fields: { log_index: {}, transaction_hash: {} } },
      ["log_index", "transaction_hash"]
    );
    await dbAdapter.ensureUniqueness(
      sync.tableName.concat("ERC1155Approvals"),
      { fields: { log_index: {}, transaction_hash: {} } },
      ["log_index", "transaction_hash"]
    );
...
const InitializeSyncsPlugin = async (parseObject, express, secret, syncs) => {
      ...
      try {
        ...
        for (const log of req.body.logs) {
          ...
          if (sync && abi) {
            ...
            updates[sync.tableName.concat("Logs")].push({
              filter,
              update,
              upsert: true,
            });
        ...
        if (req.body.txs?.length > 0) {
          for (const tx of req.body.txs) {
            ...
            if (sync) {
              ...
              updates[sync.tableName.concat("Txs")].push({
                filter,
                update,
                upsert: true,
              });
        ...
        if (req.body.erc20Transfers?.length > 0) {
          for (const erc20Transfer of req.body.erc20Transfers) {
            ...
            updates["ERC20Transfers"].push({
              filter: {
                transaction_hash: erc20Transfer.transactionHash,
                log_index: erc20Transfer.logIndex,
              },
              update: erc20Transfer,
              upsert: true,
            });
          }
        }
        for (const erc20Approval of req.body.erc20Approvals) {
          ...
          updates["ERC20Approvals"].push({
            filter: {
              transaction_hash: erc20Approval.transactionHash,
              log_index: erc20Approval.logIndex,
            },
            update: erc20Approval,
            upsert: true,
          });
        }
        for (const nftTransfer of req.body.nftTransfers) {
          if (!updates["NFTTransfers"]) updates["NFTTransfers"] = [];
          updates["NFTTransfers"].push({
            filter: {
              transaction_hash: nftTransfer.transactionHash,
              log_index: nftTransfer.logIndex,
            },
            update: nftTransfer,
            upsert: true,
          });
        ...
        for (const erc721Approval of req.body.nftApprovals.ERC721) {
          ...
          updates["ERC721Approvals"].push({
            filter: {
              transaction_hash: erc721Approval.transactionHash,
              log_index: erc721Approval.logIndex,
            },
            update: erc721Approval,
            upsert: true,
          });
        }
        for (const erc1155Approval of req.body.nftApprovals.ERC1155) {
          if (!updates["ERC1155Approvals"]) updates["ERC1155Approvals"] = [];
          updates["ERC1155Approvals"].push({
            filter: {
              transaction_hash: erc1155Approval.transactionHash,
              log_index: erc1155Approval.logIndex,
            },
            update: erc1155Approval,
            upsert: true,
          });
        }
